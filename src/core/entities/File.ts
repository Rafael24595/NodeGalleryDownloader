...
import { FileExposed } from "./FileExposed";
import { Directory } from "./Directory";

export class File extends System {

  public static open(path:string, flags?: fs.OpenMode){
    flags = flags ? flags : "a";

    let dirName = Uri.dirName(path);
    Directory.createDirSync(dirName);

    return FileExposed.expose(path, flags);
  }
